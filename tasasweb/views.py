            ...
            if user.userprofile.tipoUsr==0:
                ...
                if unico_padr:
                  try:
                        ...
                        if padr:
                          LOGIN_REDIRECT_URL = reverse('tadese:ver_cuotas', kwargs={'idp':padr.id_padron})
                        else:
                          LOGIN_REDIRECT_URL = reverse('tadese:padrones_responsable')
                  except:
                        padr = None   
                else:
                  LOGIN_REDIRECT_URL = reverse('tadese:padrones_responsable')
            elif user.userprofile.tipoUsr==1:
                LOGIN_REDIRECT_URL = reverse('tadese:padrones_estudio')
