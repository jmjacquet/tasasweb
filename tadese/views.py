...
class DreiLiquidarCreateView(VariablesMixin,CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse('tadese:ver_cuotas', kwargs={'idp':cuota.id_padron})
...
class DreiLiquidarUpdateView(VariablesMixin,UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse('tadese:ver_cuotas', kwargs={'idp':boleta.id_cuota.id_padron})
...
def calcularPunitorios(request,c,boleta=None,importe=0):
    # try:
        ...
        nobonificable = Decimal(cuota.tributo.nobonificable or 0)
                        
        bonificable = Decimal(cuota.tributo.bonificacion or 0)
        if (not bonificable)or(bonificable<0):
            bonificable = Decimal(0)
        ...
        if cuota.tributo.id_tributo==6 :
            ...
            bonificable = Decimal(bonificable)
            nobonificable = Decimal(nobonificable)
            #si no está vecida
            if (hoy<=vencimiento):                
            ...
            importe = (saldo - nobonificable) * (1-(bonificable/100)) + nobonificable            
            importe1 =  importe + porc1
            punit1 = importe1        
...
def EliminarBoleta(request,idb):
    ...
    try:        
        ...
        return HttpResponseRedirect(reverse('tadese:ver_cuotas', kwargs={'idp':cuota.id_padron}))
    except Exception as e:        
...
def suscripcion_alta(request,idp):
    ...
    return HttpResponseRedirect(reverse('tadese:ver_cuotas', kwargs={'idp':idp}))    
